...
class DayCentModel(nn.Module):
    def __init__(self, input_dim, init_dim, year_dim, hidden_dim=128, latent_dim=32, lstm_layers=2, **kwargs):
        ...
        # Project static context, including the previous year's terminal SOMSC.
        # SOMSC is a stock variable, so this state is the dominant anchor.
        self.init_proj = nn.Linear(init_dim + year_dim + 1, latent_dim)
    ...
    def _init_somsc_heads(self):
        # Start exactly at the copy-previous-state baseline so training only
        # has to learn meaningful corrections to the carbon stock dynamics.
    ...
    def forward(self, batch):
        ...
        # ---- global representation (init + year + previous SOMSC state) ----
        global_latent = self.init_proj(
            torch.cat([init_cond, year_enc, prev_somsc], dim=-1)
        )  # (B, latent)
        ...
        for m, (start, end) in enumerate(ranges):
            ...
            # Use month-specific attention and head to predict the monthly delta.
            pooled, attn = self.somsc_attns[m](h, mask=mask)
            head_input = torch.cat([pooled, current_val.unsqueeze(-1)], dim=-1)
            delta = self.somsc_heads[m](head_input).squeeze(-1)  # (B,)
